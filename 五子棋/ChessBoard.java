...
//绘制   
  public void paintComponent(Graphics g){  
      ...
      int imgWidth= img.getWidth(this);//获得图片的宽度  
      int imgHeight=img.getHeight(this);//获得图片的高度   
      int FWidth=getWidth();  //获得窗口的宽度
      int FHeight=getHeight();//获得窗口的高度   
      ...
      for(int i=0;i<chessCount;i++){  
         ...
         // g.fillOval(xPos-Point.DIAMETER/2, yPos-Point.DIAMETER/2,   
          ...
          if(i==chessCount-1){//如果是最后一个棋子   
              ...
              g.drawRect(xPos-Point.DIAMETER/2, yPos-Point.DIAMETER/2,  34, 35);  //画矩形
          }  
      }  
  }  
  ...
  public void mousePressed(MouseEvent e){//鼠标在组件上按下时调用   
      ...
      String colorName=isBlack?"黑棋":"白棋";  //条件运算符
        
      //将鼠标点击的坐标位置转换成网格索引   
      xIndex=(e.getX()-MARGIN+GRID_SPAN/2)/GRID_SPAN;  //e.getX()获得绝对坐标
      yIndex=(e.getY()-MARGIN+GRID_SPAN/2)/GRID_SPAN;  //GRID_SPAN代表格的宽度
